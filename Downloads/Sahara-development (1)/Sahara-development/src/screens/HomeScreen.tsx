...
const [isCaregiver, setIsCaregiver] = useState(false);
  ...
  const [selectedTab, setSelectedTab] = useState("all"); // "all" | "taken" | "missed" | "pending" (caregiver only)
...
const caregiverRes = await fetch(`${API_BASE}/api/caregiver/${parsedUser._id}/is-caregiver`);
    ...
    setIsCaregiver(caregiverData.isCaregiver);
      ...
      const [medsRes, pendingRes] = await Promise.all([
        fetch(`${API_BASE}/api/caregiver/${parsedUser._id}/dependents-meds`),
        fetch(`${API_BASE}/api/caregiver/${parsedUser._id}/pending-medications`),
      ]);
      const medsData = await medsRes.json();
      const pendingData = await pendingRes.json();
      setDependentsMeds(Array.isArray(medsData) ? medsData : []);
      setPendingMeds(Array.isArray(pendingData) ? pendingData : []);
    } else {
  ...
  const pendingVerifyCount = pendingMeds.length;
  ...
  const filteredMeds = medsToShow.filter((med) => {
    ...
    if (selectedTab === "pending") return false; // pending shown from pendingMeds
    return true;
  });

  const medsToList = selectedTab === "pending" && dashboardMode === "caregiver" ? pendingMeds : filteredMeds;

  const verifyMedication = async (medId: string, action: "verify" | "reject") => {
    try {
      const res = await fetch(
        `${API_BASE}/api/caregiver/verify-medication/${medId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ caregiverUserId: user._id, action }),
        }
      );
      if (res.ok) {
        setPendingMeds((prev) => prev.filter((m) => m._id !== medId));
        fetchUserData();
      } else {
        const data = await res.json();
        Alert.alert(t("common.error") || "Error", data.error || "Failed to verify");
      }
    } catch (err) {
      ...
      {dashboardMode === "personal"
        ...
        <View style={styles.tabs}>
          {[
            ...
            ...(dashboardMode === "caregiver" && isCaregiver
              ? [{ key: "pending", label: t("caregiver.pendingVerification"), count: pendingVerifyCount }]
              : []),
          ].map((tab) => (
            <TouchableOpacity
              key={tab.key}
        ...
        {medsToList.map((med) => (
          ...
          >
            <TouchableOpacity
              style={[styles.savedMedContainer, darkMode && { backgroundColor: "#2C2C2C" }]}
              onPress={() => selectedTab !== "pending" && goToDetail(med)}
              activeOpacity={selectedTab === "pending" ? 1 : 0.7}
            >
              {dashboardMode === "caregiver" && (
                <Text style={styles.dependentName}>{med.dependentName}</Text>
              )}
              <Text style={[styles.medName, darkMode && { color: "#fff" }]}>{med.name}</Text>
              <Text style={darkMode ? { color: "#ccc" } : {}}>{med.dose}</Text>
              <Text style={darkMode ? { color: "#ccc" } : {}}>{med.frequency}</Text>

              {selectedTab === "pending" ? (
                <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
                  <TouchableOpacity
                    style={[styles.takeButton, { flex: 1 }]}
                    onPress={() => verifyMedication(med._id, "verify")}
                  >
                    <Text style={{ color: "#fff" }}>{t("caregiver.verify")}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.rejectButton, { flex: 1 }]}
                    onPress={() => verifyMedication(med._id, "reject")}
                  >
                    <Text style={{ color: "#fff" }}>{t("caregiver.reject")}</Text>
                  </TouchableOpacity>
                </View>
              ) : (
                <>
                  {med.status === "missed" && <Text style={styles.missed}>MISSED</Text>}
                  {med.verificationStatus === "pending" && (
                    <Text style={[styles.missed, { color: "#FF9500" }]}>
                      {t("caregiver.pendingVerification")}
                    </Text>
                  )}
                  {med.status !== "taken" && med.verificationStatus !== "pending" && (
