...
const fetchUser = async () => {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/medications/delete-requests/${caregiverId}`);
    const data = await res.json();
...
const approveTheme = async (id: string) => {
  const res = await fetch(
    `${API_BASE}/api/caregiver/approve-theme/${id}`,
    { method: "POST" }
  );
...
const rejectMedDelete = async (requestId: string) => {
  ...
  return (
    <ScrollView style={{ padding: 20 }}>
      <Text style={{ fontSize: 18, fontWeight: "700", marginTop: 20, marginBottom: 10 }}>
  ...
  <Text style={{ color: "#888" }}>No pending acceptance requests.</Text>
      ...
      {requests.map((req: any) => (
        ...
        >
          ...
          <TouchableOpacity
  onPress={() =>
    navigation.navigate("EditMedicationRequestScreen", { request: req })
  }
>
...
{medDeleteRequests.map((req: any) => (
  <View key={req._id} style={{ backgroundColor: "#fff", padding: 15, borderRadius: 10, marginBottom: 10 }}>
    <Text style={{ fontWeight: "bold" }}>
      Dependent: {req.dependent?.name}
    </Text>
...
{themeRequests.map((req: any) => (
  ...
  >
    ...
    <TouchableOpacity
      onPress={() => approveTheme(req._id)}
    >
      <Text style={{ color: "green" }}>Approve</Text>
    </TouchableOpacity>

    <TouchableOpacity
      onPress={() => rejectTheme(req._id)}
    >
      <Text style={{ color: "red" }}>Reject</Text>
    </TouchableOpacity>
  </View>
