...
import { View, Text, TouchableOpacity, StyleSheet, Alert, ActivityIndicator, Animated, StatusBar } from 'react-native';
...
export const savePrescription = async (
  ...
  try {
    ...
    if (forDependentId) {
      ...
      const alreadyExists = dependentList.some((p: any) => p.id === newEntry.id);
      if (!alreadyExists) {
        dependentList.unshift(newEntry);
        await AsyncStorage.setItem(dependentPersonalKey, JSON.stringify(dependentList));
      }
    } else {
...
export default function ScanPrescriptionScreen() {
  const pulseAnim = useRef(new Animated.Value(1)).current; const scanLineAnim = useRef(new Animated.Value(0)).current; const btn1Scale = useRef(new Animated.Value(1)).current; const btn2Scale = useRef(new Animated.Value(1)).current;
...
useEffect(() => {
 ...
 return (
  <View style={styles.container}>
    ...
    {isProcessing ? (
      <Animated.View style={[styles.scanner, { transform: [{ scale: pulseAnim }] }]}>
