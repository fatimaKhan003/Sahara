...
export  default function ScanPrescriptionScreen() {
  ...
  const processImage = async (localUri: string) => {
    ...
    try {
      ...
      formData.append("file", {           // ← Python FastAPI expects "file"
        uri: localUri,
      ...
      navigation.navigate("ConfirmMedicationScreen", {
        imageUri: localUri,
        backendImageUri: "",
        detectedName: data.medicines?.[0]?.name || "",   // backward compat
        ocrMedicines: data.medicines || [],               // ← the key fix
      });
      ...
      // Navigate anyway so user can enter manually
      navigation.navigate("ConfirmMedicationScreen", {
        imageUri: localUri,
        backendImageUri: "",
        detectedName: "",
