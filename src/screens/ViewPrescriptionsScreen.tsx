...
import {
  ...
  StyleSheet, Alert, Modal, Dimensions, SafeAreaView, SectionList,
  ActivityIndicator,
} from 'react-native';
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      if (userIsCaregiver) {
        ...
        const personalList: Prescription[] = personalRaw ? JSON.parse(personalRaw) : [];

        
        const caregiverKey  = getCaregiverKey(parsedUser._id);
        const caregiverRaw  = await AsyncStorage.getItem(caregiverKey);
        const caregiverList: Prescription[] = caregiverRaw ? JSON.parse(caregiverRaw) : [];
...
for (const dep of depData) {
    ...
    depMap[dep._id].items.push(
      ...
      if (userIsDependent) {
        try {
          ...
          const cgRes  = await fetch(`${API_BASE}/api/caregiver/my-caregiver/${parsedUser._id}`);
  ...
  const renderItem = ({ item, section }: { item: Prescription; section: SectionData }) => (
    <TouchableOpacity
  ...
  const renderSectionHeader = ({ section }: { section: SectionData }) => (
    <View style={[
      ...
      <View style={[
        ...
        {
          backgroundColor: section.sectionKey === 'personal'
            ? (darkMode ? '#1a2a3a' : '#E8F0FE')
            : section.sectionKey === 'caregiver_added'
              ? (darkMode ? '#2a1a3a' : '#F3E8FF')
              : (darkMode ? '#1a3a2a' : '#E8F8EE'),
        },
      ]}>
        <Ionicons
          name={
            section.sectionKey === 'personal' ? 'person-outline'
            : section.sectionKey === 'caregiver_added' ? 'heart-outline'
            : 'people-outline'
          }
          size={16}
          color={
            section.sectionKey === 'personal' ? '#1E5AF2'
            : section.sectionKey === 'caregiver_added' ? '#7C3AED'
            : '#059669'
          }
        />
        <Text style={[
...
const styles = StyleSheet.create({
  ...
  card: {
    ...
    marginBottom: 10, padding: 10, elevation: 2,
