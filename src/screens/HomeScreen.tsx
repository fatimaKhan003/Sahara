...
const dependentList = useMemo(() => {
  ...
  const medRes = await fetch(`${API_BASE}/api/medications/requests/${parsedUser._id}`);
  const medRequests = await medRes.json();

  const themeRes = await fetch(`${API_BASE}/api/caregiver/theme-requests/${parsedUser._id}`);
          ...
          onPress: async()=>
          {
            try{
              ...
              if(data.isDependent)
              {
                ...
                Alert.alert("Request Sent","Your caregiver will be notified to approve this deletion.");
              }
              else
              {await fetch(`${API_BASE}/api/medications/${id}`, { method: "DELETE" });
              await cancelMedicationNotifications(id);
              if(dashboardMode==="personal"){setMedications((prev)=>prev.filter((m)=>m._id!==id));}
              else{setDependentsMeds((prev)=>prev.filter((m)=>m._id!==id));}
