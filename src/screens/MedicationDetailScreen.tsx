...
useEffect(() => {
  const checkDependent = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/caregiver/is-dependent/${parsedUser._id}`);
        ...
        try {
          if (isDependent) {
            ...
            await fetch(`${API_BASE}/api/medications/request-delete`, {
              ...
              body: JSON.stringify({ medicationId: med._id, userId: parsedUser._id }),
            });
            Alert.alert("Request Sent", "Your caregiver will review and approve the deletion.", [
